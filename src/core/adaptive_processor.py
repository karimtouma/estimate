...
try:
    ...
    from ..utils.language_router import LanguageDetectionResult, LanguageRouter
...
class AdaptiveProcessor(PDFProcessor):
    ...
    def __init__(self, config: Optional[Config] = None):
        ...
        logger.info(
            f"Adaptive Processor initialized (dynamic schemas: {'enabled' if self.enable_dynamic_schemas else 'disabled'})"
        )
    ...
    def _extract_text_samples_from_discovery(self, discovery_result) -> List[str]:
        ...
        if (
            hasattr(discovery_result, "discovered_patterns")
            and discovery_result.discovered_patterns
        ):
        ...
        if (
            hasattr(discovery_result, "nomenclature_system")
            and discovery_result.nomenclature_system
        ):
    ...
    async def _trigger_gepa_optimization_if_needed(
        self, analysis_result: Dict[str, Any], discovery_result: Dict[str, Any]
    ) -> None:
        ...
        if len(self.analysis_history) >= gepa_threshold:
            logger.info(
                f"🧬 GEPA TRIGGER: {len(self.analysis_history)} analyses accumulated - starting comprehensive prompt optimization..."
            )

            # Run GEPA optimization in background
            asyncio.create_task(
                self._run_comprehensive_gepa_optimization(
                    discovery_result.get("industry_domain", "construction_documents")
                )
            )
    ...
    async def _collect_current_prompts(self, domain: str) -> Dict[str, str]:
        ...
        for analysis_type in [
            ...
            prompt, _ = self._get_analysis_config(
                analysis_type,
                {
    ...
    def comprehensive_analysis_adaptive(
        ...
        try:
            ...
            if enable_discovery and self.enable_dynamic_schemas:
                logger.info(
                    "🔍 FASE 1: Running enhanced discovery with dynamic schema integration..."
                )
                ...
                try:
                    ...
                    enhanced_discovery_result = main_loop.run_until_complete(
                        enhanced_discovery.enhanced_initial_exploration(
                            sample_size=10, pdf_uri=file_uri
                        )
                    )
                    ...
                    logger.info(
                        f"  - Dynamic types discovered: {len(enhanced_discovery_result.discovered_element_types)}"
                    )
                    logger.info(
                        f"  - Auto-registered types: {len(enhanced_discovery_result.auto_registered_types)}"
                    )

                    # FASE 1.5: Language Detection and Prompt Optimization
                    logger.info(
                        "🌐 FASE 1.5: Detecting document language for prompt optimization..."
                    )

                    # Extract text samples from discovery for language detection
                    text_samples = self._extract_text_samples_from_discovery(
                        enhanced_discovery_result
                    )
                    ...
                    logger.info(
                        f"  - Primary language: {language_detection_result.primary_language}"
                    )
                    ...
                    logger.info(
                        f"  - Technical terms: {len(language_detection_result.technical_terminology)}"
                    )
            ...
            if "data_extraction" in enabled_types:
                analysis_data = self.analyze_document(
                    file_uri, "data_extraction", discovery_result_dict
                )
                    ...
                    try:
                        page_map = loop.run_until_complete(
                            enhanced_discovery.create_complete_page_map(
                                main_topics=list(
                                    enhanced_discovery_result.discovered_patterns.get(
                                        "patterns", []
                                    )
                                ),
                                pdf_uri=file_uri,
                    ...
                    try:
                        page_map = loop.run_until_complete(
                            discovery.create_complete_page_map(
                                main_topics=(
                                    discovery_result_dict.get("discovered_patterns", {}).get(
                                        "patterns", []
                                    )
                                    if discovery_result_dict
                                    else []
                    ...
                    try:
                        ...
                        try:
                            gepa_loop.run_until_complete(
                                self._trigger_gepa_optimization_if_needed(
                                    analysis_result_dict, discovery_result_dict
                                )
                            )
                        finally:
                            gepa_loop.close()
