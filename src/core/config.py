...
try:
    ...
    try:
        ...
        raise ImportError("TOML support required. Install with: pip install tomli") from None
...
class AnalysisConfig:
    ...
    enabled_types: List[str] = field(
        default_factory=lambda: ["general", "sections", "data_extraction"]
    )
    ...
    def _populate_config_objects(self) -> None:
        ...
        if "analysis" in self._raw_data:
            ...
            self.analysis = AnalysisConfig(
                enabled_types=analysis_data.get(
...
"""
    ...
    def print_summary(self) -> None:
        ...
        print(
            f"🔑 API Key: {'✅ Configured' if self.api.gemini_api_key and self.api.gemini_api_key != 'your_api_key_here' else '❌ Not configured'}"
        )
