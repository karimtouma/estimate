...
def analyze(
    ...
    try:
        ...
        if hasattr(config, "analysis"):
            click.echo(
                f"🔍 Debug: analysis.enable_dynamic_schemas = {getattr(config.analysis, 'enable_dynamic_schemas', 'NOT_FOUND')}"
            )
        ...
        click.echo(
            f"🧠 Using {processor_name} Processor (dynamic schemas: {'enabled' if enable_dynamic_schemas else 'disabled'})"
        )
        ...
        with PDFProcessor(config) as processor:
            ...
            while True:
                try:
                    ...
                    if results and "error" not in results[0]:
                        ...
                        error_msg = (
                            results[0].get("error", "Unknown error") if results else "No response"
                        )
            ...
            if conversation_results:
                ...
                chat_data = {
                    ...
                    "session_timestamp": (
                        processor.get_conversation_history()[0].get("timestamp")
                        if processor.get_conversation_history()
                        else None
                    ),
...
@cli.command()
@click.option("--pattern", "-p", default="*.json*", help="File pattern to match (default: *.json*)")
@click.pass_context
...
@cli.command()
@click.option("--days", "-d", default=30, help="Maximum age of files to keep (default: 30 days)")
@click.option(
