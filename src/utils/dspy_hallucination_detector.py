...
class DetectRepetitiveHallucination(dspy.Signature):
    ...
    pattern_found: str = dspy.OutputField(
        desc="The repetitive pattern if found, empty string otherwise"
    )
    ...
    def detect_and_clean(self, text: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if detection.has_repetition and detection.pattern_found:
                ...
                # Clean the text
                cleaning = self.clean_text(text=text, pattern=detection.pattern_found)
    ...
    def validate_field(self, field_name: str, data: Any, max_items: int = 50) -> Dict[str, Any]:
        ...
        try:
            ...
            validation = self.validate_data(
                data=data_str, field_name=field_name, max_items=max_items
            )
            ...
            if not validation.is_valid:
                ...
                return {"valid": False, "data": cleaned, "issues": validation.issues}

            return {"valid": True, "data": data, "issues": None}

        except Exception as e:
            logger.error(f"Error validating field {field_name}: {e}")
            return {"valid": False, "data": data, "issues": str(e)}
...
def validate_page_classification(classification: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for field in text_fields:
        if field in cleaned and isinstance(cleaned[field], str):
            ...
            if had_hallucination:
                logger.warning(
                    f"🔴 Hallucination in page {cleaned.get('page_number', '?')} {field}"
                )
