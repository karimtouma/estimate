...
def setup_logging(config: Config) -> None:
    ...
    logger.info(
        f"Logging configured - Level: {config.processing.log_level}, Container: {config.is_container_environment()}"
    )
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            logger.info(
                ...
                extra={
                    "extra_fields": {"endpoint": endpoint, "method": method, "status": "started"}
                },
            )
