...
class AdaptiveQuestionGenerator:
    ...
    def generate_questions(
        ...
        templates = self.domain_question_templates.get(domain_category, self.domain_question_templates['generic'])
        ...
        logger.info(f"Generated {len(final_questions)} adaptive questions for {domain_category} domain")
    ...
    def _classify_domain(self, industry_domain: str, document_type: str) -> str:
        ...
        if any(keyword in domain_lower for keyword in ['process', 'chemical', 'petrochemical', 'manufacturing']):
            return 'process'
        elif any(keyword in doc_type_lower for keyword in ['p&id', 'process', 'piping']):
            return 'process'
        
        # Naval domain (check before general architecture)
        elif any(keyword in domain_lower for keyword in ['naval', 'marine', 'maritime', 'shipbuilding']):
            return 'naval'
        ...
        # Construction/AEC domain
            ...
            return 'construction'
        elif any(keyword in doc_type_lower for keyword in ['blueprint', 'plan', 'elevation', 'section']):
            return 'construction'
...
def generate_adaptive_questions(discovery_result: Dict[str, Any], max_questions: int = 8) -> List[str]:
    """
