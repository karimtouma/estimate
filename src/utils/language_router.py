...
class LanguageRouter:
    ...
    async def detect_document_language(
        self, text_samples: List[str], use_ai_analysis: bool = True
        ...
        if use_ai_analysis and self.gemini_client:
            ai_detection = await self._ai_language_detection(
                text_samples[:5]
            )  # Limit samples for API
            if ai_detection:
    ...
    async def _ai_language_detection(self, text_samples: List[str]) -> Optional[Dict[str, Any]]:
        ...
        try:
            response_text = self.gemini_client.generate_text_only_content(
                prompt=prompt, response_schema=self._get_language_detection_schema()
            )
    ...
    def _get_language_detection_schema(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "properties": {
                ...
                "reasoning": {"type": "string", "description": "Explanation of language detection"},
            },
    ...
    def _merge_detection_results(
        self, pattern_scores: Dict[str, float], ai_detection: Dict[str, Any]
    ) -> Dict[str, float]:
    ...
    def optimize_prompt_for_language(
        ...
        if language_result.technical_terminology:
            terminology_instruction = self._get_terminology_instruction(
                language_result.technical_terminology, optimal_language
            )
        else:
        ...
        if language_result.region_indicators:
            regional_instruction = self._get_regional_instruction(
                language_result.region_indicators, optimal_language
            )
        ...
        logger.info(
            f"Prompt optimized for {optimal_language} with {len(language_result.technical_terminology)} technical terms"
        )
    ...
    def _get_spanish_instruction(self, language_result: LanguageDetectionResult) -> str:
        ...
        if "mexico" in language_result.region_indicators:
            ...
            region in language_result.region_indicators
            for region in ["spain", "argentina", "colombia"]
        ):
            base_instruction += (
                " Usa terminología técnica latinoamericana/española cuando sea apropiada."
            )
    ...
    def _get_english_instruction(self, language_result: LanguageDetectionResult) -> str:
        ...
        if "usa" in language_result.region_indicators:
            ...
            base_instruction += (
                " Use Canadian technical terminology and standards when appropriate."
            )
...
async def detect_and_optimize_prompt(
    ...
    optimized_prompt = router.optimize_prompt_for_language(
        base_prompt, language_result, analysis_type
    )
