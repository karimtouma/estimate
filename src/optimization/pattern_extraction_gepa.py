...
class PromptEvolutionResult:
    ...
    def get_improvement_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "convergence_rate": len([p for p in self.performance_history if p > 0.1]) / len(self.performance_history) if self.performance_history else 0
        }
...
class PatternExtractionGEPA:
    ...
    async def evolve_pattern_extraction_prompts(
        ...
        """
        logger.info(f"Starting GEPA evolution for {len(base_prompts)} prompts in domain: {target_domain}")
        ...
        for prompt_type, base_prompt in base_prompts.items():
            ...
            logger.info(f"✅ {prompt_type} evolution complete: {evolution_result.performance_improvement:.3f} improvement")
    ...
    async def _evolve_single_prompt(
        ...
        for generation in range(self.max_generations):
            logger.debug(f"GEPA Generation {generation + 1}/{self.max_generations} for {prompt_type}")
            ...
            if improvement >= target_improvement:
                logger.info(f"Target improvement {target_improvement:.3f} reached in generation {generation + 1}")
                break
        ...
        return PromptEvolutionResult(
            ...
            specialization_score=await self._calculate_specialization_score(best_prompt, target_domain),
    ...
    def _get_evaluation_schema(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "required": ["pattern_extraction", "domain_specificity", "structure_recognition", "nomenclature_accuracy", "overall_usefulness"]
        }
    ...
    async def _calculate_specialization_score(self, prompt: str, domain: str) -> float:
        ...
        try:
            response = self.gemini_client.generate_text_only_content(
                ...
                response_schema=self._get_specialization_schema()
            )
    ...
    async def _save_evolution_results(
        self,
        results: Dict[str, PromptEvolutionResult],
        domain: str
    ) -> None:
...
def create_pattern_extraction_gepa(config: Config, gemini_client: Optional[GeminiClient] = None) -> PatternExtractionGEPA:
