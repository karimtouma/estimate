...
class GEPAClassificationEnhancer:
    ...
    async def enhance_classification(
        ...
        improvement = self._calculate_improvement(base_classification, best_candidate) if base_classification else 0.0
        ...
        logger.debug(f"🧬 GEPA enhancement complete: {best_candidate.type_name} (judge score: {best_candidate.judge_score:.3f})")
            ...
            if hasattr(response, 'candidates') and response.candidates:
                for i, candidate in enumerate(response.candidates):
                    if hasattr(candidate, 'content') and candidate.content.parts:
                        ...
                        parsed_candidate = self._parse_candidate_response(candidate_text, f"candidate_{i+1}")
                        if parsed_candidate:
    ...
    def _parse_candidate_response(self, response_text: str, candidate_id: str) -> Optional[ClassificationCandidate]:
    ...
    def _calculate_specificity_score(self, type_name: str, reasoning: str) -> float:
        ...
        generic_penalty = sum(0.2 for term in specificity_indicators['generic_terms'] if term in type_lower)
        
        # Bonus for specific terms
        specific_bonus = sum(0.1 for term in specificity_indicators['specific_terms'] if term in type_lower)
        
        # Extra bonus for highly specific terms
        highly_specific_bonus = sum(0.2 for term in specificity_indicators['highly_specific'] if term in type_lower)
        ...
        base_score = 0.5
        final_score = base_score - generic_penalty + specific_bonus + highly_specific_bonus + length_bonus + reasoning_bonus
    ...
    def _get_classification_schema(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "properties": {
                "type_name": {
                    "type": "string",
                    "description": "Highly specific element type name"
                },
                "category": {
                    "type": "string",
    ...
    def _get_judge_schema(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "properties": {
                "best_candidate_id": {
                    "type": "string",
                    "description": "ID of the best candidate"
                },
                "candidate_evaluations": {
                    "type": "array",
                    "items": {
    ...
    def _calculate_consensus_score(self, candidates: List[ClassificationCandidate]) -> float:
        ...
        category_consensus = categories.count(max(set(categories), key=categories.count)) / len(categories)
    ...
    def _fallback_judge_evaluation(self, candidates: List[ClassificationCandidate]) -> Dict[str, Any]:
        """Fallback judge evaluation when AI judge fails."""
        
        # Simple scoring based on confidence and specificity
        for candidate in candidates:
            candidate.judge_score = (candidate.confidence * 0.6) + (candidate.specificity_score * 0.4)
    ...
    def get_enhancement_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_processing_time": statistics.mean([r["processing_time"] for r in recent_records]),
...
def create_gepa_classification_enhancer(config: Config, gemini_client: Optional[GeminiClient] = None) -> GEPAClassificationEnhancer:
