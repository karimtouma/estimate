...
class ComprehensiveGEPAResult:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_improvement": (
                statistics.mean(self.performance_improvements.values())
                if self.performance_improvements
                else 0.0
            ),
            "best_improvement": (
                max(self.performance_improvements.values())
                if self.performance_improvements
                else 0.0
            ),
            "total_improvement": self.total_improvement,
            "optimization_time_minutes": self.optimization_time / 60,
            "prompts_needing_optimization": len(
                [t for t in self.test_results_before.values() if t.is_optimization_worthy()]
            ),
        }
...
class ComprehensiveGEPASystem:
    ...
    async def optimize_all_dynamic_prompts(
        ...
        logger.info(
            f"🧬 Starting comprehensive GEPA optimization for {len(current_prompts)} prompts"
        )
        ...
        logger.info(
            f"🎯 {len(prompts_to_optimize)}/{len(current_prompts)} prompts need optimization"
        )
        ...
        for prompt_id, prompt_text in prompts_to_optimize.items():
            ...
            logger.info(
                f"✅ {prompt_id} optimized: {improvement:.3f} improvement in {generations} generations"
            )
    ...
    async def _test_all_prompts(
        self, prompts: Dict[str, str], test_documents: List[Dict[str, Any]]
    ...
    async def _test_single_prompt(
        self, prompt_id: str, prompt_text: str, test_documents: List[Dict[str, Any]]
    ) -> PromptTestResult:
        ...
        for doc in test_documents:
            try:
                ...
                response = self.gemini_client.generate_text_only_content(
                    prompt=test_prompt, response_schema=self._get_test_schema()
                )
    ...
    async def _evolve_single_prompt(
        ...
        for generation in range(self.max_generations):
            ...
            if improvement >= self.improvement_target:
                logger.info(
                    f"Target improvement reached for {prompt_id} in generation {generation + 1}"
                )
                break
    ...
    async def _mutate_prompt(self, prompt: str, prompt_type: str, domain: str) -> str:
        ...
        try:
            evolved_prompt = self.gemini_client.generate_text_only_content(
                prompt=mutation_prompt, model="gemini-2.5-pro"
            )
    ...
    async def _create_next_generation(
        self, population_scores: List[Tuple[str, float]], prompt_type: str, domain: str
    ) -> List[str]:
        ...
        while len(next_generation) < self.population_size:
            if len(population_scores) >= 2:
                ...
                if population_scores:
                    mutated = await self._mutate_prompt(
                        population_scores[0][0], prompt_type, domain
                    )
                    next_generation.append(mutated)
                else:
                    break
    ...
    async def _crossover_prompts(
        self, parent1: str, parent2: str, prompt_type: str, domain: str
    ) -> str:
    ...
    async def _save_optimization_results(
        self, result: ComprehensiveGEPAResult, domain: str
    ) -> None:
        ...
        try:
            ...
            results_file = (
                output_dir / f"comprehensive_gepa_optimization_{domain}_{int(time.time())}.json"
            )
...
def create_comprehensive_gepa_system(
