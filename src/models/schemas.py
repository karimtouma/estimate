...
class DocumentAnalysis(BaseModel):
    ...
    summary: str = Field(
        description="Executive summary of the document content", min_length=50, max_length=1000
    )
    main_topics: List[str] = Field(
        description="Main topics identified in the document", min_items=1, max_items=10
    )
    key_insights: List[str] = Field(
        description="Key insights extracted from the document", min_items=1, max_items=15
    )
    document_type: DocumentType = Field(description="Identified document type")
    confidence_score: float = Field(
        description="Confidence level of the analysis (0-1)", ge=0.0, le=1.0
    )
...
class SectionAnalysis(BaseModel):
    ...
    section_title: str = Field(
        description="Title or heading of the section", min_length=1, max_length=200
    )
    content_summary: str = Field(
        description="Summary of the section content", min_length=20, max_length=500
    )
    important_data: List[str] = Field(
        description="Important data points found in the section", max_items=20
    )
    questions_raised: List[str] = Field(
        description="Questions or concerns raised by the content", max_items=10
    )
    section_type: Optional[str] = Field(
        description="Type of section (introduction, methodology, results, etc.)", default=None
    )
...
class DataExtraction(BaseModel):
    ...
    entities: List[str] = Field(
        description="Named entities (people, organizations, locations)", max_items=50
    )
    dates: List[str] = Field(description="Relevant dates mentioned in the document", max_items=30)
    numbers: List[str] = Field(
        description="Important numbers, metrics, and statistics (max 100 chars each)", max_items=40
    )
    references: List[str] = Field(
        description="Citations, references, and external sources", max_items=25
    )
    key_terms: Optional[List[str]] = Field(
        description="Technical terms and domain-specific vocabulary", max_items=30, default=[]
    )
...
class QuestionAnswer(BaseModel):
    ...
    question: str = Field(description="The question asked", min_length=5)
    answer: str = Field(description="The generated answer", min_length=10)
    confidence: float = Field(description="Confidence in the answer (0-1)", ge=0.0, le=1.0)
    sources: Optional[List[str]] = Field(
        description="Sources or sections that informed the answer", default=[]
    )
    follow_up_questions: Optional[List[str]] = Field(
        description="Suggested follow-up questions", max_items=5, default=[]
    )
    question_index: Optional[int] = Field(
        description="Index of the question in a sequence", ge=1, default=None
    )
...
class ComprehensiveAnalysisResult(BaseModel):
    ...
    discovery_analysis: Optional[dict] = Field(
        default=None, description="Discovery phase analysis results"
    )
    dynamic_schema_results: Optional[dict] = Field(
        default=None, description="Dynamic schema discovery and classification results"
    )
    page_map: Optional[DocumentPageMap] = Field(
        default=None, description="Complete page-by-page analysis and categorization"
    )
