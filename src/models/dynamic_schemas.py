...
class AdaptiveElementType(BaseModel):
    ...
    base_category: CoreElementCategory = Field(description="Core category for structural organization")
    
    # Specific type (dynamic, discovered)
    specific_type: str = Field(description="Specific element type, can be discovered dynamically")
    
    # Contextual information
    domain_context: Optional[str] = Field(description="Domain context (industrial, residential, etc.)", default=None)
    industry_context: Optional[str] = Field(description="Industry context (petrochemical, aerospace, etc.)", default=None)
    
    # Discovery metadata
    discovery_confidence: float = Field(description="Confidence in type classification", ge=0.0, le=1.0, default=1.0)
    discovery_method: DiscoveryMethod = Field(description="Method used to discover/classify type", default=DiscoveryMethod.HYBRID_ANALYSIS)
    is_dynamically_discovered: bool = Field(description="Whether this type was discovered dynamically", default=False)
    
    # Type hierarchy
    parent_type: Optional[str] = Field(description="Parent type in hierarchy", default=None)
    specificity_level: int = Field(description="Level of specificity (0=general, higher=more specific)", default=0)
    
    # Quality indicators
    reliability_score: float = Field(description="Reliability of this classification", ge=0.0, le=1.0, default=1.0)
...
class DynamicElementRegistry:
    ...
    def __init__(self, persistence_path: Optional[Path] = None):
        ...
        self.category_counts: Dict[CoreElementCategory, int] = {cat: 0 for cat in CoreElementCategory}
    ...
    def register_discovered_type(
        ...
        **kwargs
        ...
        try:
            ...
            logger.info(f"Registered new type: {normalized_name} (category: {base_category.value}, confidence: {discovery_confidence:.3f})")
    ...
    def create_adaptive_element_type(
        self, 
        type_name: str,
        base_category: Optional[CoreElementCategory] = None,
        **kwargs
    ...
    def get_types_by_category(self, category: CoreElementCategory) -> List[ElementTypeDefinition]:
        ...
        return [type_def for type_def in self.discovered_types.values() 
    ...
    def _calculate_similarity(self, query: str, type_name: str, type_def: ElementTypeDefinition) -> float:
        ...
        try:
            ...
            for type_name, type_data in data.get('discovered_types', {}).items():
                type_def = ElementTypeDefinition(
                    ...
                    discovery_method=DiscoveryMethod(type_data.get('discovery_method', 'hybrid_analysis')),
