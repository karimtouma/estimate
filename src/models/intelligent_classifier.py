...
class IntelligentTypeClassifier:
    ...
    async def classify_element(
        self, element_data: Dict[str, Any], context: Optional[Dict[str, Any]] = None
        ...
        try:
            ...
            if self.gepa_enhancer:
                ...
                gepa_result = await self.gepa_enhancer.enhance_classification(
                    element_info=element_info, context=context, base_classification=best_result
                )
                ...
                logger.debug(
                    f"🧬 GEPA enhanced: {best_result.classified_type} (consensus: {gepa_result.consensus_score:.3f})"
                )
            ...
            if (
                ...
                and best_result.confidence >= self.auto_register_threshold
            ):
            ...
            best_result.requires_validation = (
                best_result.confidence < self.validation_threshold or best_result.is_new_discovery
            )
    ...
    async def _classify_by_pattern_matching(
        ...
        for type_name, pattern in patterns.items():
            confidence = self._calculate_pattern_confidence(pattern, visual_features, text_features)
    ...
    async def _classify_by_ai_reasoning(
        ...
        try:
            ...
            response_text = self.gemini_client.generate_text_only_content(
                prompt=prompt, response_schema=self._get_classification_schema()
            )
    ...
    def _get_classification_schema(self) -> Dict[str, Any]:
        ...
        return {
            "type": "object",
            "properties": {
                "type_name": {"type": "string", "description": "Specific element type name"},
                "category": {
                    "type": "string",
    ...
    async def _create_fallback_classification(
        ...
        try:
            ...
            response_text = self.gemini_client.generate_text_only_content(
                prompt=fallback_prompt, response_schema=self._get_classification_schema()
            )
    ...
    def _enhance_with_ensemble(
        self, best_result: ClassificationResult, all_results: List[ClassificationResult]
    ) -> ClassificationResult:
            ...
            stats.update(
                {
                    ...
                    "gepa_enhancement_rate": self.gepa_enhancement_count
                    / max(1, self.classification_count),
