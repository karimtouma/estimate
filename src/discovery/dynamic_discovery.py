...
class DynamicPlanoDiscovery:
    ...
    async def evolve_extraction_prompts(
        self, target_domain: str = "construction_documents"
    ) -> bool:
        """
    ...
    def _record_discovery_for_gepa(self, discovery_result: Dict[str, Any]) -> None:
        ...
        logger.debug(
            f"Discovery recorded for GEPA evolution: {len(self.discovery_history)} total records"
        )
    ...
    def strategic_sampling(self, total_pages: int, sample_size: int = 10) -> List[int]:
        ...
        if total_pages <= 20:
            ...
            logger.info(
                f"📖 Medium document ({total_pages} pages): analyzing {adaptive_sample_size} pages (60%)"
            )
        else:
            ...
            logger.info(
                f"📚 Large document ({total_pages} pages): analyzing {adaptive_sample_size} pages (30%)"
            )
        ...
        complexity_samples = self._find_high_complexity_pages(
            limit=min(5, adaptive_sample_size // 3)
        )
        samples.update(complexity_samples)

        # QUARTILE sampling for even distribution
        quartiles = [total_pages // 4, total_pages // 2, (3 * total_pages) // 4]
        ...
        logger.info(
            f"🎯 EXHAUSTIVE sampling: {len(samples_list)} pages ({coverage_percent:.1f}% coverage)"
        )
        logger.info(
            f"📋 Selected pages: {samples_list[:10]}{'...' if len(samples_list) > 10 else ''}"
        )
    ...
    async def initial_exploration(
        ...
        if len(self.discovery_history) >= 3:  # Minimum for meaningful evolution
            logger.info(
                "🧬 Sufficient discoveries accumulated - triggering GEPA prompt evolution..."
            )
                ...
                try:
                    ...
                    try:
                        loop.run_until_complete(
                            self._trigger_gepa_evolution_background(result.industry_domain)
                        )
                    finally:
                        loop.close()
                except Exception as e:
    ...
    def _analyze_page_discovery(
        self, page_image: bytes, page_text: str, prompt: str, pdf_uri: str = None
    ) -> Dict[str, Any]:
    ...
    async def deep_pattern_analysis(self, discovery_result: DiscoveryResult) -> Dict[str, Any]:
        ...
        for pattern_type, pages in pattern_pages.items():
            ...
            pattern_detail = await self._analyze_pattern_detail(
                pattern_type, pages, discovery_result
            )
    ...
    async def _analyze_pattern_detail(
        self, pattern_type: str, pages: List[int], discovery_result: DiscoveryResult
    ) -> Dict[str, Any]:
        """Analyze a specific pattern in detail."""
        pattern_detail = {"type": pattern_type, "characteristics": [], "rules": [], "examples": []}
        ...
        for page_num in pages[:2]:  # Limit to avoid too many API calls
            ...
            pattern_detail["examples"].append({"page": page_num, "analysis": analysis})
    ...
    def _build_relationship_graph(self, cross_references: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for ref in cross_references:
            ...
            if source and target:
                graph["edges"].append(
                    {"from": source, "to": target, "type": ref.get("type", "references")}
                )
    ...
    async def _understand_visual_language(
    ...
    async def create_complete_page_map(self, pdf_uri: str, main_topics: List[str]) -> dict:
        ...
        page_batches = [
            list(range(i, min(i + batch_size, self.total_pages)))
        ...
        async def classify_page_batch(batch_pages: List[int]) -> List[dict]:
            async with semaphore:
                try:
                    ...
                    return [
                        self._get_default_page_classification(p, main_topics) for p in batch_pages
                    ]
        ...
        category_distribution = self._calculate_category_distribution(
            all_page_classifications, main_topics
        )
        ...
        logger.info(
            f"🎯 Complete page map created in {processing_time:.1f}s for {self.total_pages} pages"
        )
    ...
    async def _preload_all_pages(self):
        ...
        logger.info(
            f"✅ All pages cached in {cache_time:.1f}s - {len(self.page_cache)} pages ready"
        )

    def _classify_pages_batch(
        self, pdf_uri: str, prompt: str, batch_pages: List[int], main_topics: List[str]
    ) -> List[dict]:
            ...
            try:
                ...
                for classification in classifications:
                    ...
                    if 0 <= page_num < self.total_pages:
                        classification["complexity_score"] = self._calculate_visual_complexity(
                            page_num
                        )
                ...
                logger.warning(
                    f"Failed to parse page classification response for batch {batch_pages}"
                )
    ...
    def _calculate_category_distribution(
        self, page_classifications: List[dict], main_topics: List[str]
    ) -> dict:
    ...
    def _analyze_topic_coverage(
        ...
        for topic in main_topics:
            topic_pages = [
                ...
                if p.get("primary_category") == topic or topic in p.get("secondary_categories", [])
            ]
            ...
            if topic_pages:
                ...
                coverage[topic] = {
                    ...
                    "avg_confidence": sum(p.get("confidence", 0) for p in topic_pages)
                    / len(topic_pages),
                    "page_range": {
    ...
    def _initialize_smart_cache(self):
        ...
        cache_time = time.time() - start_time
        logger.info(
            f"✅ Smart cache initialized in {cache_time:.2f}s - {len(self.page_cache)} pages cached"
        )

    async def _analyze_pages_parallel(
        self, sample_pages: List[int], exploration_prompt: str
    ) -> DiscoveryResult:
        """
        ...
        logger.info(
            f"🔄 Starting PARALLEL analysis of {len(sample_pages)} pages with intelligent batching"
        )
        ...
        batches = [
            sample_pages[i : i + batch_size] for i in range(0, len(sample_pages), batch_size)
        ]
        ...
        logger.info(
            f"✅ Parallel analysis completed in {parallel_time:.1f}s for {len(sample_pages)} pages"
        )
    ...
    def _analyze_batch_discovery(
        self, pdf_uri: str, prompt: str, sample_pages: List[int]
    ) -> Dict[str, Any]:
        """
    ...
    async def _process_batch_discovery(
        self, discovery: Dict[str, Any], sample_pages: List[int]
    ) -> DiscoveryResult:
