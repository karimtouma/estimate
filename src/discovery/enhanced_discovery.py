...
try:
    ...
    from ..models.dynamic_schemas import (
        ...
        DiscoveryMethod, get_dynamic_registry, initialize_dynamic_registry
    )
...
class EnhancedDynamicDiscovery(DynamicPlanoDiscovery):
    ...
    async def enhanced_initial_exploration(
        ...
        logger.info(f"  - Dynamic types discovered: {len(enhanced_result.discovered_element_types)}")
        logger.info(f"  - Auto-registered types: {len(enhanced_result.auto_registered_types)}")
        logger.info(f"  - Classification confidence: {enhanced_result.classification_performance.get('average_confidence', 0):.3f}")
    ...
    async def _enhance_discovery_with_dynamic_schemas(
        ...
        for element in unique_elements:
            try:
                ...
                classification_confidences[classification_result.classified_type] = classification_result.confidence
                
                # Track auto-registered types
                if classification_result.is_new_discovery and not classification_result.requires_validation:
                    auto_registered.append(classification_result.classified_type)
                    self.auto_registered_count += 1
                
                logger.debug(f"Classified element: {classification_result.classified_type} "
        ...
        enhanced_result.classification_performance = self.intelligent_classifier.get_classification_stats()
        ...
        if classification_confidences:
            ...
            enhanced_result.confidence_score = (
                (base_result.confidence_score * 0.6) + (avg_classification_confidence * 0.4)
            )
        
        return enhanced_result
    
    async def _extract_unique_elements_from_discovery(self, base_result: DiscoveryResult) -> List[Dict[str, Any]]:
        """
    ...
    async def analyze_element_relationships(self, elements: List[AdaptiveElementType]) -> Dict[str, List[str]]:
        """
        ...
        for element in elements:
            ...
            if element.base_category == CoreElementCategory.STRUCTURAL:
                ...
                architectural_elements = category_groups.get(CoreElementCategory.ARCHITECTURAL.value, [])
    ...
    def _are_elements_related(self, element1: AdaptiveElementType, element2: AdaptiveElementType) -> bool:
        """
