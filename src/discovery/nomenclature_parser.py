    ...
    def _analyze_components(self, codes: List[str]) -> Dict[str, Any]:
        ...
        return {
            ...
            "number_range": {
                ...
                "average": sum(components["numbers"]) / len(components["numbers"]) if components["numbers"] else 0
            },
    ...
    def _derive_rules(self, codes: List[str], patterns: Dict[str, Any]) -> List[Dict[str, str]]:
        ...
        for prefix, meaning in prefix_meanings.items():
            rules.append({
                ...
                "confidence": "high" if len([c for c in codes if c.startswith(prefix)]) > 3 else "medium"
            })
    ...
    def _infer_prefix_meanings(self, codes: List[str]) -> Dict[str, str]:
        ...
        for prefix, codes_list in prefix_groups.items():
            ...
            if numbers:
                ...
                if count > 10:
                    meanings[prefix] = f"{prefix}-Series (frequent, {count} instances, range {num_range})"
                elif count > 5:
