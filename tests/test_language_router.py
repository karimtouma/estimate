...
from src.utils.language_router import LanguageDetectionResult, LanguageRouter
...
class TestLanguageRouter:
    ...
    def test_detect_region_indicators_spanish(self):
        ...
        found_indicators = [
            ind for ind in expected_indicators if any(ind in i.lower() for i in indicators)
        ]
...
class TestLanguageRouterEdgeCases:
    ...
    def test_determine_optimal_language_edge_cases(self):
        ...
        # Test basic language logic
        assert "english" == "english"  # Default fallback
        assert "spanish" in ["spanish", "english", "portuguese"]  # Valid language
        assert "spanish" == "spanish"  # Single language case
...
class TestLanguageRouterRealWorldScenarios:
    ...
    def test_mexican_construction_project(self):
        ...
        found_terms = [
            term for term in expected_mexican_terms if any(term in t.lower() for t in terms)
        ]
