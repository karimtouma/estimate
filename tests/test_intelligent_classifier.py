...
from src.models.dynamic_schemas import (
    DynamicElementRegistry, CoreElementCategory, DiscoveryMethod
)
from src.models.intelligent_classifier import (
    IntelligentTypeClassifier, ClassificationResult
)
    ...
    def test_extract_nomenclature_codes(self):
        ...
        for text, expected_codes in text_samples:
            ...
            for code in expected_codes:
                assert any(code in extracted_code for extracted_code in extracted), f"Expected {code} in {extracted}"
...
class TestRealWorldClassification:
    ...
    async def test_industrial_equipment_classification(self):
        ...
        for element in industrial_elements:
            result = await self.classifier.classify_element(element["data"], {
                "document_domain": "industrial",
                "industry_context": "petrochemical"
            })
    ...
    async def test_structural_element_classification(self):
        ...
        for element in structural_elements:
            ...
            assert result is not None
            assert result.base_category == CoreElementCategory.STRUCTURAL
            assert any(expected in result.classified_type.lower() 
